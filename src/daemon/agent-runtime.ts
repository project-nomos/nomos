    ...
    if (this.gwsAccounts && this.gwsAccounts.length > 0) {
      ...
      parts.push(
        [
          ...
          `  Authorized accounts:\n${accountList}`,
          `  **Usage**: Run \`npx gws <service> <resource> <method> --params '<JSON>'\` via the Bash tool.`,
          `  **Examples**:`,
          `    - List emails: \`npx gws gmail users messages list --params '{"userId":"me","maxResults":5}'\``,
          `    - Read email: \`npx gws gmail users messages get --params '{"userId":"me","id":"<msgId>","format":"full"}'\``,
          `    - Send email: \`npx gws gmail users messages send --params '{"userId":"me"}' --json '{"raw":"<base64>"}'\``,
          `    - List events: \`npx gws calendar events list --params '{"calendarId":"primary","maxResults":5}'\``,
          `    - List files: \`npx gws drive files list --params '{"pageSize":10}'\``,
          `    - Search: \`npx gws gmail users messages list --params '{"userId":"me","q":"from:someone subject:topic"}'\``,
          `  **Multi-account**: The active account is the default. To switch, re-auth is needed.`,
          `  **Tip**: Use \`npx gws schema <service.resource.method>\` to check available params.`,
        ].join("\n"),
      );
    }
